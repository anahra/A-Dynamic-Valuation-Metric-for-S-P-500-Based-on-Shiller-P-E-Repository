...
def plot_strategy_results(data):
    ...
    fig.add_trace(go.Scatter(x=data['Date'], y=data['Benchmark_Portfolio'], mode='lines', name='DCA Portfolio Value', line=dict(color='blue'), hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    fig.add_trace(go.Scatter(x=data['Date'], y=data['Strategy_Portfolio'], mode='lines', name='dDCA Portfolio Value', line=dict(color='orange'), hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    ...
    fig.add_trace(go.Scatter(x=data['Date'], 
                            ...
                            line=dict(color='cyan', dash='dot'), hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    fig.add_trace(go.Scatter(x=data['Date'], 
                            y=data['Strategy_Portfolio'] + data['Strategy_Cashflow'].clip(lower=0),  # Only add positive cash
                            mode='lines', name='dDCA Total Value', 
                            line=dict(color='red', dash='dot'), hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    fig.update_layout(
    ...
    fig_monthly.add_trace(go.Bar(x=monthly_data['Date'], y=monthly_benchmark, name='DCA Monthly Cashflow', marker_color='blue', hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    fig_monthly.add_trace(go.Bar(x=monthly_data['Date'], y=monthly_strategy, name='dDCA Monthly Cashflow', marker_color='orange', hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    ...
    fig_cashflow.add_trace(go.Scatter(x=data['Date'], y=data['Benchmark_Cashflow'], mode='lines', name='DCA Cashflow', line=dict(color='blue'), hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    fig_cashflow.add_trace(go.Scatter(x=data['Date'], y=data['Strategy_Cashflow'], mode='lines', name='dDCA Cashflow', line=dict(color='orange'), hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    fig_cashflow.add_trace(go.Scatter(x=data['Date'], y=data['Benchmark_MaxNegativeCashflow'], mode='lines', name='DCA Capital Required', line=dict(color='red'), hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    fig_cashflow.add_trace(go.Scatter(x=data['Date'], y=data['Strategy_MaxNegativeCashflow'], mode='lines', name='dDCA Capital Required', line=dict(color='green'), hovertemplate='%{x|%d %b %Y}: $%{y:,.0f}<extra></extra>'))
    ...
    fig_percentage_profit.add_trace(go.Scatter(x=data['Date'], y=data['Benchmark_Percentage_Profit'], mode='lines', name='DCA % Profit', line=dict(color='green'), hovertemplate='%{x|%d %b %Y}: %{y:.1f}%<extra></extra>'))
    fig_percentage_profit.add_trace(go.Scatter(x=data['Date'], y=data['Strategy_Percentage_Profit'], mode='lines', name='dDCA % Profit', line=dict(color='purple'), hovertemplate='%{x|%d %b %Y}: %{y:.1f}%<extra></extra>'))
    ...
    fig_drawdown.add_trace(go.Scatter(x=data['Date'], y=benchmark_drawdown, mode='lines', name='DCA Drawdown', line=dict(color='cyan'), hovertemplate='%{x|%d %b %Y}: %{y:.1f}%<extra></extra>'))
    fig_drawdown.add_trace(go.Scatter(x=data['Date'], y=strategy_drawdown, mode='lines', name='dDCA Drawdown', line=dict(color='red'), hovertemplate='%{x|%d %b %Y}: %{y:.1f}%<extra></extra>'))
    ...
    # Portfolio Composition Plot (Equity vs Cash)
    fig_composition = go.Figure()
    
    # Calculate Cash Balance: Strategy_Cashflow - Strategy_MaxNegativeCashflow (which is negative)
    # This represents the cash held from sales that hasn't been reinvested yet
    cash_balance = data['Strategy_Cashflow'] - data['Strategy_MaxNegativeCashflow']
    
    # Calculate Total Assets (Portfolio Value + Cash Balance)
    total_assets = data['Strategy_Portfolio'] + cash_balance
    
    # Calculate Percentages (handle division by zero)
    equity_pct = np.where(total_assets > 0, (data['Strategy_Portfolio'] / total_assets) * 100, 0)
    cash_pct = np.where(total_assets > 0, (cash_balance / total_assets) * 100, 0)
    
    fig_composition.add_trace(go.Scatter(
        x=data['Date'], y=equity_pct, 
        mode='lines', 
        name='Equity %', 
        stackgroup='one', # Enable stacking
        line=dict(width=0, color='orange'),
        fillcolor='rgba(255, 165, 0, 0.7)', # Orange for Equity
        hovertemplate='%{x|%d %b %Y}: %{y:.1f}%<extra></extra>'
    ))
    fig_composition.add_trace(go.Scatter(
        x=data['Date'], y=cash_pct, 
        mode='lines', 
        name='Cash %', 
        stackgroup='one', # Enable stacking
        line=dict(width=0, color='green'),
        fillcolor='rgba(0, 128, 0, 0.5)', # Green for Cash
        hovertemplate='%{x|%d %b %Y}: %{y:.1f}%<extra></extra>'
    ))
    
    # Update layout with merged settings
    layout_settings = chart_settings.copy()
    if 'yaxis' in layout_settings:
        layout_settings['yaxis'] = layout_settings['yaxis'].copy()
        layout_settings['yaxis'].update(dict(range=[0, 100]))
    else:
        layout_settings['yaxis'] = dict(range=[0, 100])

    fig_composition.update_layout(
        title="dDCA Portfolio Composition: Equity vs. Cash",
        xaxis_title="Date",
    ...
    return [fig, fig_monthly, fig_cashflow, fig_percentage_profit, fig_drawdown, fig_composition]
